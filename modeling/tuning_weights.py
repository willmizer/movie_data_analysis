# Tuning Weights Program
# This script tunes the weights of various features in a movie recommendation system using Optuna for hyperparameter optimization.
# It computes embeddings for movie overviews and keywords, builds a feature matrix, and evaluates the precision of recommendations based on different weight configurations.
...
# function to recommend movies based on a seed title
...
# function to score a seed title based on its recommendations
def score_seed(seed, relevant, X_w, k=3):
    recs = recommend(seed, X_w, k=k)
    return precision_at_k(recs, relevant, k=k)

# function to evaluate weights against ground truth
